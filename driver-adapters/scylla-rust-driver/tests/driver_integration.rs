...
use driver_counterpart::driver_integration::UniversalDriver;
...
fn build_config(socket_path: PathBuf, contact_points: Vec<String>) -> DriverConfig {
    DriverConfig {
        socket_path,
        contact_points,
        keyspace: env::var("SCYLLA_KEYSPACE").ok(),
        inflight_limit: 8,
    }
}
...
async fn rejects_unsupported_opcode_and_connects_to_scylla() -> anyhow::Result<()> {
    ...
    let config = build_config(socket_path.clone(), contact_points);
    ...
    loop {
        ...
        attempts += 1;
        if attempts > 10 {
            anyhow::bail!("socket was not created in time");
        }
...
async fn connects_and_reads_release_version() -> anyhow::Result<()> {
    ...
    let config = build_config(socket_path, contact_points);
...
async fn runs_queries_over_unix_socket() -> anyhow::Result<()> {
    ...
    let mut driver = UniversalDriver::new(socket_path.clone(), socket_path.clone());
    driver.start().await?;
    let session = driver
