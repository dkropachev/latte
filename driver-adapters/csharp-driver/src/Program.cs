...
public class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        var contactPoints = Environment.GetEnvironmentVariable("LATTE_DRIVER_CONTACT_POINTS")
            ?? "127.0.0.1";
        ...
        logger.LogInformation("Default contact points: {ContactPoints}", contactPoints);
        logger.LogInformation("Max in-flight requests: {Inflight}", inflight);

        // Create session registry
        var sessions = new SessionRegistry(loggerFactory, contactPoints);
