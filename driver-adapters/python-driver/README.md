...
| `LATTE_DRIVER_CONTACT_POINTS` | `127.0.0.1:9042` | Comma-separated contact points |
...
1. Verify the database is running and accessible:
   ...
   cqlsh <host> <port>
   ```
2. Check the contact points are correct:
   ```bash
   echo $LATTE_DRIVER_CONTACT_POINTS
   ```
3. If using Docker, ensure the container can reach the database:
