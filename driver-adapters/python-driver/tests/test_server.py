...
class TestRateLimitedLogging:
    ...
    def test_first_occurrence_logged(self, caplog):
        ...
        assert _error_counts["test_error"] == 1
    ...
    def test_repeated_errors_rate_limited(self, caplog):
        ...
        _rate_limited_error_log("test_error", "Test error message")
        assert _error_counts["test_error"] == 2
    ...
    def test_time_interval_triggers_log(self, caplog, monkeypatch):
        ...
        _error_last_logged["test_error"] = time.time() - ERROR_LOG_INTERVAL - 1
    ...
    def test_different_errors_tracked_separately(self, caplog):
        ...
        assert _error_counts["error_a"] == 1
        assert _error_counts["error_b"] == 1
...
class TestWriterLoop:
    ...
    async def test_writer_loop_writes_frames(self, server):
        """Writer loop should write encoded frames to the stream."""
        # Create mock writer
        writer = MagicMock()
        ...
        # Verify frames were encoded correctly
        calls = writer.write.call_args_list
        assert calls[0][0][0] == encode_frame(frame1)
        assert calls[1][0][0] == encode_frame(frame2)
