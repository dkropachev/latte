...
| `LATTE_DRIVER_CONTACT_POINTS` | `127.0.0.1:9042` | Comma-separated host:port list |
...
**`LATTE_DRIVER_CONTACT_POINTS`** (Required)
- Comma-separated list of database nodes: `host1:port1,host2:port2`
- Port defaults to 9042 if omitted
- In Docker with `--network host`: Use `127.0.0.1:9042`
- In Docker Desktop (Mac/Windows): Use `host.docker.internal:9042`

**`LATTE_DRIVER_KEYSPACE`** (Optional)
- Default keyspace for the session
- Can also be set via session parameter in CREATE_SESSION
...
fn load_config():
    config = Config {
        ...
        contact_points: env("LATTE_DRIVER_CONTACT_POINTS", "127.0.0.1:9042")
            .split(",")
            .map(parse_host_port),
        keyspace: env_optional("LATTE_DRIVER_KEYSPACE"),
...
# Default contact points for database
# Use 127.0.0.1:9042 if running with --network host
# Use host.docker.internal:9042 for Docker Desktop (Mac/Windows)
ENV LATTE_DRIVER_CONTACT_POINTS=127.0.0.1:9042
...
**With `--network host` (Linux, recommended for benchmarking):**
```bash
...
**With Docker Desktop (Mac/Windows):**
```bash
mkdir -p /tmp/latte-driver
...
```bash
# Latte connects to the socket on the host path
...
The `<driver-folder-name>` must match the adapter's directory name under `driver-adapters/`.
...
- [ ] `LATTE_DRIVER_CONTACT_POINTS` environment variable
- [ ] `LATTE_DRIVER_KEYSPACE` environment variable (optional)
...
- [ ] Sets `ENV LATTE_DRIVER_CONTACT_POINTS=127.0.0.1:9042`
