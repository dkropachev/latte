...
public final class Protocol {
   ...
   * @param body the frame body bytes
   */
  public record Frame(
      byte version, byte flags, short stream, byte opcode, byte[] body) {}
   ...
   *
   * <p>Provides reusable BytesWriter instances to reduce allocation pressure in performance-critical
   * code paths. Each thread gets its own cached writer to avoid synchronization overhead.
   *
   * <p>The default capacity can be configured via the {@code latte.bytesWriterPoolCapacity}
   * system property (default: 4096 bytes).
   *
   * <p>Note: This pool tracks nesting depth to handle recursive encoding (e.g., nested collections).
   * Only the outermost acquire() returns the pooled writer; nested calls get new instances.
   *
   * <p>Usage:
   * <pre>{@code
  ...
  public static class BytesWriterPool {
     ...
     *
     * <p>If this is a nested call (e.g., encoding a collection element), a new writer
     * is created to avoid corrupting the outer call's data.
     *
  ...
  public static class FrameBuilder {
     ...
     * @return the complete frame bytes
     */
    public static byte[] buildPreparedFrame(short stream, @NonNull String statementKey) {
      BytesWriter writer = BytesWriterPool.acquire();
      try {
     ...
     * @return the complete frame bytes
     */
    public static byte[] buildErrorFrame(
        short stream, int errorCode, @NonNull String message) {
      BytesWriter writer = BytesWriterPool.acquire();
      try {
  ...
  public static class PooledBytesWriter implements AutoCloseable {
     ...
     *
     * <p>Uses direct (off-heap) buffers by default for better I/O performance. This can be
     * disabled by setting the system property {@code latte.directBuffers=false}.
     *
     * @param initialCapacity the initial buffer capacity in bytes
     */
    public PooledBytesWriter(int initialCapacity) {
      this.buffer = USE_DIRECT_BUFFERS
