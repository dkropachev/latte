...
def plot_workload(ax, data: dict, percentile: str, show_legend: bool = True, log_scale: bool = False):
    ...
    failed_bar_height = max(successful_totals) * 0.1 if successful_totals else 1.0
    ...
    # Only add overhead bars for non-failed adapters
    overhead_values = [0 if bar_is_failed[i] else bar_overhead_values[i] for i in range(len(bar_labels))]
    ax.bar(x, overhead_values, width, bottom=display_driver_values,
           color=[color_overhead if not bar_is_failed[i] else color_failed for i in range(len(bar_labels))],
           edgecolor="black")
