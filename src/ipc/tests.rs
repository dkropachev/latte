...
mod tests {
    ...
    fn test_packed_format_saves_space() {
        ...
        let vectors: Vec<CqlValue> = (0..10)
            .map(|_| {
                let elements: Vec<CqlValue> = (0..768).map(|j| CqlValue::Float(j as f32)).collect();
                CqlValue::Vector(elements)
            })
            .collect();
    ...
    /// Test to verify that the protocol files are in sync.
    /// This compares the key protocol constants between the main crate and driver adapter.
    /// If this test fails, ensure both protocol.rs files have matching definitions.
    #[test]
    fn test_protocol_files_in_sync() {
        use std::fs;
        use std::path::Path;

        let main_protocol = Path::new("src/ipc/protocol.rs");
        let adapter_protocol = Path::new("cql-adapters/scylla-rust-driver/src/protocol.rs");

        // Skip test if files don't exist (e.g., in CI without full checkout)
        if !main_protocol.exists() || !adapter_protocol.exists() {
            eprintln!("Skipping protocol sync test: protocol files not found");
            return;
        }

        let main_content = fs::read_to_string(main_protocol).expect("Failed to read main protocol");
        let adapter_content =
            fs::read_to_string(adapter_protocol).expect("Failed to read adapter protocol");

        // Extract and compare key protocol constants
        let extract_constant = |content: &str, name: &str| -> Option<String> {
            for line in content.lines() {
                if line.contains(&format!("const {name}")) || line.contains(&format!("{name} =")) {
        ...
        for constant in constants_to_check {
            let main_val = extract_constant(&main_content, constant);
            let adapter_val = extract_constant(&adapter_content, constant);

            assert_eq!(
                main_val, adapter_val,
                "Protocol constant {} mismatch between main and adapter protocol files. \
                 Ensure src/ipc/protocol.rs and cql-adapters/scylla-rust-driver/src/protocol.rs are in sync.",
                constant
            );
        }

        // Also verify Opcode enum has same variants
        let extract_opcodes = |content: &str| -> Vec<String> {
            let mut in_opcode = false;
            let mut opcodes = Vec::new();
            for line in content.lines() {
                if line.contains("enum Opcode") {
                    in_opcode = true;
                    continue;
                }
                if in_opcode {
                    ...
                    if !trimmed.is_empty()
                        ...
                        && !trimmed.starts_with('#')
                    {
                        opcodes.push(trimmed.to_string());
                    }
                }
            }
            opcodes
        ...
        let main_opcodes = extract_opcodes(&main_content);
        let adapter_opcodes = extract_opcodes(&adapter_content);

        assert_eq!(
            main_opcodes, adapter_opcodes,
            "Opcode enum mismatch between main and adapter protocol files. \
             Ensure src/ipc/protocol.rs and cql-adapters/scylla-rust-driver/src/protocol.rs are in sync."
        );
    }
}
