...
impl IpcClient {
    /// Connect to the driver counterpart at the given socket path.
    pub async fn connect(socket_path: impl AsRef<Path>) -> Result<Self> {
        ...
        let pending: Arc<DashMap<i16, PendingRequest>> = Arc::new(DashMap::new());
    ...
    pub async fn batch(
        ...
        let driver_latency = if response.body.len() >= 12 {
            ...
            // DEBUG: Log received batch latency
            tracing::debug!(
                body_len = body_len,
...
/// - Flushes when buffer is large (32KB) or when no more requests are immediately available
    ...
    *slice = rest;
    ...
    let value = match type_code {
        ...
        0x0006 => {
            ...
            CqlValue::Decimal(scylla::value::CqlDecimal::from_signed_be_bytes_slice_and_exponent(
                mantissa_bytes,
                scale,
            ))
        }
        ...
        0x000D => {
            // text/varchar - avoid double allocation for valid UTF-8
            CqlValue::Text(
                String::from_utf8(data.to_vec())
                    .unwrap_or_else(|e| String::from_utf8_lossy(e.as_bytes()).into_owned()),
            )
...
pub fn encode_value(value: &CqlValue, buf: &mut BytesMut) -> Result<()> {
    ...
    match value {
        ...
        CqlValue::List(elements) => {
            ...
            // Encode elements into a temporary buffer to get total length
            let mut data_buf = BytesMut::new();
            data_buf.put_u16(subtype);

            // For list<vector<float, N>>, we need to also encode vector subtype and dimension
            if subtype == type_codes::VECTOR {
                data_buf.put_u16(type_codes::FLOAT); // vector element type
            ...
            for element in elements {
                match element {
                    CqlValue::Vector(vec_elements) => {
                        ...
                        for ve in vec_elements {
                            match ve {
                                ...
                                CqlValue::Double(d) => {
                                    data_buf.extend_from_slice(&(*d as f32).to_be_bytes())
                                }
                                _ => bail!("vector element must be float"),
                            }
                        }
                    ...
                    _ => {
                        // For other types, encode recursively (simplified - just handle common cases)
                        let mut elem_buf = BytesMut::new();
                        encode_value(element, &mut elem_buf)?;
                        // Skip the type code and use just the data portion
                        let elem_data = &elem_buf[2..]; // skip type_code
                        data_buf.extend_from_slice(elem_data);
                    }
                }
            }

            let list_len: i32 = data_buf
                .len()
                .try_into()
                .map_err(|err| anyhow!("list data too long: {err}"))?;
            buf.put_i32(list_len);
        ...
        CqlValue::Map(entries) => {
            ...
            let mut data_buf = BytesMut::new();
            data_buf.put_u16(key_type);
            data_buf.put_u16(value_type);

            let n_entries: i32 = entries
                .len()
                .try_into()
                .map_err(|err| anyhow!("too many map entries: {err}"))?;
            data_buf.put_i32(n_entries);

            for (key, value) in entries {
                // Encode key
                let mut key_buf = BytesMut::new();
                encode_value(key, &mut key_buf)?;
                let key_data = &key_buf[2..]; // skip type_code
                data_buf.extend_from_slice(key_data);

                // Encode value
                let mut val_buf = BytesMut::new();
                encode_value(value, &mut val_buf)?;
                let val_data = &val_buf[2..]; // skip type_code
                data_buf.extend_from_slice(val_data);
            }

            let map_len: i32 = data_buf
                .len()
                .try_into()
        ...
        CqlValue::Tuple(elements) => {
            ...
            for elem in elements {
                match elem {
                    Some(val) => {
                        let mut elem_buf = BytesMut::new();
                        encode_value(val, &mut elem_buf)?;
                        let elem_data = &elem_buf[2..]; // skip type_code
                        data_buf.extend_from_slice(elem_data);
                    }
                    None => {
                        data_buf.put_i32(-1); // null
                    }
                }
            }

            let tuple_len: i32 = data_buf
                .len()
                .try_into()
                .map_err(|err| anyhow!("tuple data too long: {err}"))?;
            buf.put_i32(tuple_len);
            buf.extend_from_slice(&data_buf);
        }
            ...
            for (_, field_value) in fields {
                match field_value {
                    Some(val) => {
                        ...
                        encode_value(val, &mut field_buf)?;
                        let field_data = &field_buf[2..]; // skip type_code
                        data_buf.extend_from_slice(field_data);
                    }
                    None => {
                        data_buf.put_i32(-1); // null
                    }
...
fn read_string(slice: &mut &[u8]) -> Result<String> {
    ...
    *slice = rest;
    // Avoid double allocation for valid UTF-8
