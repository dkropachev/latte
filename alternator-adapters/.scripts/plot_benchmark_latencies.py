#!/usr/bin/env python3
"""
Generate a PNG chart comparing latencies across Alternator (DynamoDB) adapters.

This script reads JSON benchmark reports from latte and produces bar charts
showing latency comparisons for:
- Pure latte (native HTTP client, no adapter) - single green bar
...
Usage:
    ...
    python3 plot_benchmark_latencies.py --output chart.png \
        ...
        --adapter alternator-client-golang:reports/alternator-client-golang.json

    # Multiple workloads:
    python3 plot_benchmark_latencies.py --output chart.png \
        --workload crud:reports/pure-latte-crud.json:alternator-client-golang=reports/alternator-client-golang-crud.json \
        --workload query:reports/pure-latte-query.json:alternator-client-golang=reports/alternator-client-golang-query.json
"""
...
def collect_workload_data(
    ...
    # Store reports for potential mean fallback
    adapter_reports = {}
    ...
    for adapter_name, report_path in adapters:
        ...
        adapter_reports[adapter_name] = report
    ...
    # If percentile-based overhead is exactly 0 (due to histogram bucket quantization),
    # fall back to mean-based overhead calculation
    ipc_overhead = []
    for i, (latte, adapter) in enumerate(zip(latte_side_latencies, adapter_latencies)):
        overhead = max(0, latte - adapter)
        if overhead == 0 and i < len(labels):
            # Try mean-based calculation as fallback
            adapter_name = labels[i]
            if adapter_name in adapter_reports:
                report = adapter_reports[adapter_name]
                driver_mean = get_latency_mean(report, "driver_latency")
                request_mean = get_latency_mean(report, "request_latency")
                if driver_mean is not None and request_mean is not None:
                    mean_overhead = max(0, request_mean - driver_mean)
                    if mean_overhead > 0:
                        overhead = mean_overhead
        ipc_overhead.append(overhead)
    ...
    # Add failed adapters as gray bars
...
def plot_workload(ax, data: dict, percentile: str, show_legend: bool = True, log_scale: bool = False):
    ...
    color_failed = "#9e9e9e"  # Gray for failed adapters

    # Determine colors for each bar
    driver_colors = []
    for i, (is_pure, is_failed) in enumerate(zip(bar_is_pure_latte, bar_is_failed)):
        if is_failed:
    ...
    # Calculate a reasonable height for failed bars (use max of successful bars or a default)
    successful_totals = [
        bar_driver_values[i] + bar_overhead_values[i]
        for i in range(len(bar_labels))
        if not bar_is_failed[i]
    ]
    max_total = max(successful_totals) if successful_totals else 1.0
    failed_bar_height = max_total * 0.1

    # Minimum visible height for IPC overhead (3% of max bar height)
    min_overhead_height = max_total * 0.03

    # Adjust bar values for failed adapters to show a visible gray bar
    display_driver_values = []
    for i, (driver_val, is_failed) in enumerate(zip(bar_driver_values, bar_is_failed)):
        if is_failed:
            display_driver_values.append(failed_bar_height)
        else:
            display_driver_values.append(driver_val)

    # Adjust overhead values to have minimum visible height when non-zero
    display_overhead_values = []
    for i in range(len(bar_labels)):
        if bar_is_failed[i]:
            display_overhead_values.append(0)
        elif bar_overhead_values[i] > 0:
            # Ensure overhead is at least min_overhead_height for visibility
            display_overhead_values.append(max(bar_overhead_values[i], min_overhead_height))
        else:
            display_overhead_values.append(0)

    # Create stacked bars
    ax.bar(x, display_driver_values, width, color=driver_colors, edgecolor="black")
    # Only add overhead bars for non-failed adapters with overhead
    ax.bar(x, display_overhead_values, width, bottom=display_driver_values,
           color=[color_overhead if not bar_is_failed[i] and bar_overhead_values[i] > 0 else 'none' for i in range(len(bar_labels))],
           edgecolor=["black" if not bar_is_failed[i] and bar_overhead_values[i] > 0 else "none" for i in range(len(bar_labels))])

    # Add value labels on top of bars
    for i in range(len(bar_labels)):
        if bar_is_failed[i]:
            # Show "FAILED" label for failed adapters
            ax.annotate('FAILED',
                       xy=(x[i], display_driver_values[i]),
                       xytext=(0, 3),
                       textcoords="offset points",
                       ha='center', va='bottom', fontsize=8, color='#666666', fontweight='bold')
        else:
            total = bar_driver_values[i] + bar_overhead_values[i]
            display_total = display_driver_values[i] + display_overhead_values[i]
            if total > 0:
                # Show total value on top
                ax.annotate(f'{total:.2f}',
                           xy=(x[i], display_total),
                           xytext=(0, 3),
                           textcoords="offset points",
                           ha='center', va='bottom', fontsize=8)
                # Show IPC overhead value inside the red section (if there is overhead)
                if bar_overhead_values[i] > 0:
                    overhead_center = display_driver_values[i] + display_overhead_values[i] / 2
                    ax.annotate(f'+{bar_overhead_values[i]:.2f}',
                               xy=(x[i], overhead_center),
...
def print_summary_table(workloads_data: list[dict]):
    ...
    for data in workloads_data:
        ...
        print("-" * 70)
        header = f"{'Adapter':<30} {'Driver-side':>14} {'IPC overhead':>12} {'Total':>10}"
        print(header)
        print("-" * 70)
        if pure_latte_value is not None:
            print(f"{'Pure Latte':<30} {pure_latte_value:>14.2f} {'N/A':>12} {pure_latte_value:>10.2f}")
        for i, label in enumerate(labels):
            ...
            print(f"{label:<30} {driver:>14} {overhead:>12} {total:>10}")
        for adapter_name in failed_adapters:
            print(f"{adapter_name:<30} {'FAILED':>14} {'FAILED':>12} {'FAILED':>10}")
        print("-" * 70)


def main():
    parser = argparse.ArgumentParser(
        description="Generate latency comparison chart from latte benchmark reports (Alternator/DynamoDB)"
    )
    parser.add_argument(
        "--output", "-o",
        type=Path,
    ...
    parser.add_argument(
        "--title", "-t",
        default="Alternator Adapter Latency Comparison",
        help="Chart title"
    )
    parser.add_argument(
    ...
    if args.workloads:
        ...
        for workload_arg in args.workloads:
            try:
                ...
                data = collect_workload_data(workload_name, pure_latte_path, adapters, args.percentile)
        ...
        data = collect_workload_data(workload_name, args.pure_latte, adapters, args.percentile)
        if data["bar_labels"]:
            workloads_data.append(data)
    ...
    if n_workloads == 1:
        ...
        ax.set_xlabel("Adapter", fontsize=12)
    else:
        ...
        for i, data in enumerate(workloads_data):
            ...
            if i >= n_cols * (n_rows - 1):  # Bottom row
                axes_flat[i].set_xlabel("Adapter", fontsize=10)
