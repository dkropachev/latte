...
def encode_value(value: object, type_code: int) -> bytes:
    ...
    match type_code:
        ...
        case TypeCode.VECTOR:
            # Vector - encode as raw contiguous big-endian floats
            if isinstance(value, (list, tuple)):
                result = bytearray(len(value) * 4)
                for i, item in enumerate(value):
                    _STRUCT_FLOAT.pack_into(result, i * 4, float(item))
                return bytes(result)
            if isinstance(value, bytes):
                return value
            return str(value).encode("utf-8")
                ...
                try:
                    ...
                    for item in items:
                        if isinstance(item, (list, tuple)) and item and isinstance(item[0], (int, float)):
                            # Inner vector (list of numbers) - encode as raw contiguous BE floats
                            encoded_item = bytearray(len(item) * 4)
                            for j, x in enumerate(item):
                                _STRUCT_FLOAT.pack_into(encoded_item, j * 4, float(x))
                            encoded_item = bytes(encoded_item)
                        else:
...
def _decode_set(data: bytes) -> list:
    """Decode a CQL set.

    Returns a list instead of a Python set because CQL sets may contain
    frozen collections (lists, maps) or UDTs (dicts) which are not hashable
    in Python. The cassandra-driver's SetType.serialize accepts any iterable.
    """
...
def _decode_vector_raw(data: bytes, dimension: int) -> list[float]:
    ...
    if len(data) < expected_len:
        raise ValueError(
            f"vector data too short: expected {expected_len} bytes for {dimension} dimensions, got {len(data)}"
        )
