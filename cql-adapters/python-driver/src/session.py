...
class SessionRegistry:
    ...
    def _connect_with_params(self, params: dict[str, str]) -> SessionWrapper:
        ...
        # Contact points
        contact_points_raw = params.get("contact_points", "")
...
def _expand_tuples_in_query(query: str, bind_types: list[str]) -> str:
    """No-op: the Python cassandra-driver natively supports binding tuples as single values.

    Previously this expanded single ? placeholders for tuple columns into (?, ?, ...),
    but that caused NULL tuples to be stored as (NULL, NULL, ...) instead of truly NULL.
    """
    return query
...
def _expand_values_for_tuples(
    ...
    Previously this expanded tuple values into individual elements, but that caused
    NULL tuples to be stored as (NULL, NULL, ...) instead of truly NULL.
    """
    return values
