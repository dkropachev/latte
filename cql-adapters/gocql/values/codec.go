...
func EncodeValue(value interface{}, typeCode uint16) []byte {
	...
	case []float32:
		// Vector<float, N> - encode as contiguous big-endian floats
	...
	case time.Duration:
		// gocql uses time.Duration (nanoseconds) for CQL time type
		buf := getBuffer8()
	...
	default:
		// Try to handle *int, *int64, etc. pointers first
		if encoded := handlePointerValue(value, typeCode); encoded != nil {
			return encoded
		}
		// Handle typed slices (collections from gocql like []int, []string, etc.)
		// and typed maps (like map[string]int)
		return encodeReflectValue(value, typeCode)
	}
}
...
// encodeReflectValue encodes typed slices and maps (from gocql collection scan) using reflection.
// This handles types like []int, []string, map[string]int, []map[string]interface{}, etc.
func encodeReflectValue(value interface{}, typeCode uint16) []byte {
	rv := reflect.ValueOf(value)
	switch rv.Kind() {
	case reflect.Slice:
		return encodeReflectSlice(rv, typeCode)
	case reflect.Map:
		return encodeReflectMap(rv, typeCode)
	default:
		return nil
	}
}

// encodeReflectSlice encodes a typed slice as CQL list/set format:
// [n_elements: int32] ([elem_len: int32] [elem_bytes])*
func encodeReflectSlice(rv reflect.Value, typeCode uint16) []byte {
	n := rv.Len()
	buf := make([]byte, 4, 4+n*8) // pre-allocate with room for elements
	binary.BigEndian.PutUint32(buf, uint32(n))
	for i := 0; i < n; i++ {
		elem := rv.Index(i).Interface()
		encoded := EncodeValue(elem, 0) // element type code not needed for basic types
		if encoded == nil {
			// null element
			buf = append(buf, 0xFF, 0xFF, 0xFF, 0xFF) // -1 as int32
		} else {
			lenBuf := make([]byte, 4)
...
// encodeReflectMap encodes a typed map as CQL map format:
// [n_entries: int32] ([key_len: int32] [key_bytes] [val_len: int32] [val_bytes])*
func encodeReflectMap(rv reflect.Value, typeCode uint16) []byte {
	n := rv.Len()
	buf := make([]byte, 4, 4+n*16) // pre-allocate
	binary.BigEndian.PutUint32(buf, uint32(n))
	for _, key := range rv.MapKeys() {
		keyVal := key.Interface()
		valVal := rv.MapIndex(key).Interface()

		keyEncoded := EncodeValue(keyVal, 0)
		valEncoded := EncodeValue(valVal, 0)
		...
		if keyEncoded == nil {
			...
			lenBuf := make([]byte, 4)
			binary.BigEndian.PutUint32(lenBuf, uint32(len(keyEncoded)))
			buf = append(buf, lenBuf...)
			buf = append(buf, keyEncoded...)
			PutBuffer(keyEncoded)
		}
		...
		if valEncoded == nil {
			...
			lenBuf := make([]byte, 4)
			binary.BigEndian.PutUint32(lenBuf, uint32(len(valEncoded)))
			buf = append(buf, lenBuf...)
			buf = append(buf, valEncoded...)
			PutBuffer(valEncoded)
		}
	}
	return buf
}
...
func EncodeDecimal(d *inf.Dec) []byte {
	...
	// CQL decimal wire format: [scale: int32] [unscaled: varint]
	// Both CQL and inf.Dec use the same convention: value = unscaled * 10^(-scale)
	scale := int32(d.Scale())
...
// EncodeDuration encodes a gocql.Duration to CQL duration wire format.
// The encoding uses zigzag-encoded LEB128 varints for months, days, and nanoseconds.
func EncodeDuration(d gocql.Duration) []byte {
	// Maximum size: 3 fields * 10 bytes each (max LEB128 for int64)
	buf := make([]byte, 0, 30)
	buf = appendVint(buf, int64(d.Months))
	buf = appendVint(buf, int64(d.Days))
	buf = appendVint(buf, d.Nanoseconds)
	return buf
}

// appendVint appends a zigzag-encoded LEB128 varint to the buffer.
func appendVint(buf []byte, n int64) []byte {
...
// encodeUDT encodes a UDT value (map[string]interface{}) to IPC wire format.
// Format: [n_fields: u16] [field_names_and_types...] [field_values...]
func encodeUDT(fields map[string]interface{}) []byte {
	if len(fields) == 0 {
		return []byte{0, 0} // n_fields = 0
	}
	...
	for _, k := range keys {
		...
		case int32:
			ft = uint16(TypeInt)
		case int64:
			ft = uint16(TypeBigInt)
		case string:
			ft = uint16(TypeText)
		case bool:
			ft = uint16(TypeBoolean)
		case float32:
			ft = uint16(TypeFloat)
		case float64:
			ft = uint16(TypeDouble)
		default:
			ft = uint16(TypeBlob)
		}
		buf = append(buf, byte(ft>>8), byte(ft))
	}
	...
	for _, k := range keys {
		...
		if v == nil {
			...
			encoded := EncodeValue(v, uint16(TypeBlob))
			if encoded == nil {
				buf = append(buf, 0xFF, 0xFF, 0xFF, 0xFF)
			} else {
				l := len(encoded)
...
// encodeTuple encodes a tuple value ([]interface{}) to IPC wire format.
// Format: [n_elements: u16] [element_types: u16 * n] [elements: ([length: i32] [data: bytes])*]
func encodeTuple(elements []interface{}) []byte {
	n := len(elements)
	buf := make([]byte, 0, 2+2*n+n*12)

	// n_elements
	buf = append(buf, byte(n>>8), byte(n))

	// element types - infer from Go values
	for _, elem := range elements {
		...
		case int32:
			et = uint16(TypeInt)
		case int64:
			et = uint16(TypeBigInt)
		case string:
			et = uint16(TypeText)
		case bool:
			et = uint16(TypeBoolean)
		case float32:
			et = uint16(TypeFloat)
		case float64:
			et = uint16(TypeDouble)
		default:
			et = uint16(TypeBlob)
		}
		buf = append(buf, byte(et>>8), byte(et))
	}
	...
	for _, elem := range elements {
		if elem == nil {
			...
			encoded := EncodeValue(elem, uint16(TypeBlob))
			if encoded == nil {
				buf = append(buf, 0xFF, 0xFF, 0xFF, 0xFF) // -1 = null
			} else {
				l := len(encoded)
