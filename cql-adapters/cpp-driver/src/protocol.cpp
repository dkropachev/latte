...
Buffer encode_frame(int16_t stream, Opcode opcode, const Buffer &body) {
  ...
  // Pre-allocate exact size needed (header + body)
  Buffer frame(FRAME_HEADER_SIZE + body.size());
  frame.write_byte(header.version);
  frame.write_byte(header.flags);
  frame.write_short(static_cast<uint16_t>(header.stream));
  frame.write_byte(static_cast<uint8_t>(header.opcode));
  frame.write_int(static_cast<int32_t>(header.body_length));
  frame.append(body);
  return frame;
}

Buffer build_session_created_response(uint64_t session_id) {
  Buffer body(8); // Exact size for session_id
  body.write_long(session_id);
  return body;
}

Buffer build_error_response(ErrorCode code, const std::string &message) {
  Buffer body(4 + 2 + message.size()); // int + short + string
...
Buffer build_void_result(uint64_t latency_ns) {
  Buffer body(4 + 8); // int (result kind) + long (latency)
...
Buffer build_prepared_result(const std::string &statement_key,
  ...
  // Pre-calculate: int + string + short_bytes + 4*int
  size_t capacity = 4 + (2 + statement_key.size()) + (2 + prepared_id.size()) + 16;
  Buffer body(capacity);
...
Buffer build_set_keyspace_result(const std::string &keyspace) {
  Buffer body(4 + 2 + keyspace.size()); // int + string
...
Buffer build_schema_change_result(const std::string &change_type,
  ...
  // Pre-calculate: int + 3-4 strings
  size_t capacity = 4 + (2 + change_type.size()) + (2 + target.size()) +
                    (2 + keyspace.size()) + (name.empty() ? 0 : (2 + name.size()));
  Buffer body(capacity);
