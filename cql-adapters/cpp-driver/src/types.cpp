...
static std::vector<uint8_t> reserialize_list_or_set(const uint8_t *vp, size_t vlen,
  ...
  // Reserve estimated size: 4 (n_elements) + remaining data (vlen - pos is upper bound)
  std::vector<uint8_t> out;
  out.reserve(4 + (vlen > pos ? vlen - pos : 0));
...
static std::vector<uint8_t> reserialize_map(const uint8_t *vp, size_t vlen,
  ...
  // Build output buffer with CQL format
  // Reserve estimated size: 4 (n_entries) + remaining data
  std::vector<uint8_t> out;
  out.reserve(4 + (vlen > pos ? vlen - pos : 0));
...
static std::vector<uint8_t> reserialize_udt(const uint8_t *vp, size_t vlen,
  ...
  // Reserve estimated size based on input data (field values + length prefixes)
  std::vector<uint8_t> out;
  out.reserve(vlen);
...
static std::vector<uint8_t> reserialize_tuple(const uint8_t *vp, size_t vlen,
  ...
  // Reserve estimated size: remaining data after type codes
  std::vector<uint8_t> out;
  out.reserve(vlen > pos ? vlen - pos : 0);
...
static std::vector<uint8_t> coerce_value_bytes(const uint8_t *vp, size_t vlen,
                                                CassValueType expected_type,
                                                const CassDataType *expected_data_type) {
  // Reserve capacity based on input size (output is typically same size or smaller)
  std::vector<uint8_t> out;
  out.reserve(vlen);
...
void bind_value(CassStatement *stmt, size_t index, const TypedValue &value,
  ...
  case TypeCode::TIME:
    if (len >= 8) {
      ...
      // Coerce to expected column type (Latte sends all integers as BIGINT)
      if (expected_type) {
        CassValueType exp = cass_data_type_type(expected_type);
        if (exp == CASS_VALUE_TYPE_TINY_INT) {
          cass_statement_bind_int8(stmt, index, coerce_bigint_to_tinyint(v));
          break;
        } else if (exp == CASS_VALUE_TYPE_SMALL_INT) {
          cass_statement_bind_int16(stmt, index, coerce_bigint_to_smallint(v));
          break;
        } else if (exp == CASS_VALUE_TYPE_INT) {
          cass_statement_bind_int32(stmt, index, coerce_bigint_to_int(v));
          break;
        } else if (exp == CASS_VALUE_TYPE_VARINT) {
          // Encode int64 as big-endian signed bytes for varint
          uint8_t varint_bytes[8];
          for (int b = 0; b < 8; ++b) {
            varint_bytes[b] = static_cast<uint8_t>((v >> (56 - b * 8)) & 0xFF);
          }
          cass_statement_bind_bytes(stmt, index, varint_bytes, 8);
          break;
        }
      }
  ...
  case TypeCode::DOUBLE:
    if (len >= 8) {
      ...
      double v;
      std::memcpy(&v, &bits, sizeof(v));
      // Coerce to expected column type (Latte sends all floats as DOUBLE)
      if (expected_type) {
        CassValueType exp = cass_data_type_type(expected_type);
        if (exp == CASS_VALUE_TYPE_FLOAT) {
          cass_statement_bind_float(stmt, index, coerce_double_to_float(v));
          break;
        }
      }
  ...
  case TypeCode::TEXT: {
    ...
    // Coerce text to expected column type (Latte sends dates, times, etc. as TEXT)
    if (expected_type) {
      CassValueType exp = cass_data_type_type(expected_type);
      if (exp == CASS_VALUE_TYPE_DATE) {
        // Parse "YYYY-MM-DD" → days since epoch + 2^31
        int year = 0, month = 0, day = 0;
        std::sscanf(str.c_str(), "%d-%d-%d", &year, &month, &day);
        // Days from civil date (Gregorian calendar)
        int a = (14 - month) / 12;
        int y = year + 4800 - a;
        int m = month + 12 * a - 3;
        int jdn = day + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400 - 32045;
        int epoch_jdn = 2440588; // JDN for 1970-01-01
        uint32_t date_val = static_cast<uint32_t>(jdn - epoch_jdn + (1 << 31));
        cass_statement_bind_uint32(stmt, index, date_val);
        break;
      } else if (exp == CASS_VALUE_TYPE_TIME) {
        // Parse "H:M:S" → nanoseconds since midnight
        int hours = 0, minutes = 0, seconds = 0;
        std::sscanf(str.c_str(), "%d:%d:%d", &hours, &minutes, &seconds);
        int64_t nanos = (static_cast<int64_t>(hours) * 3600 +
                         static_cast<int64_t>(minutes) * 60 +
                         static_cast<int64_t>(seconds)) * 1000000000LL;
        cass_statement_bind_int64(stmt, index, nanos);
        break;
      } else if (exp == CASS_VALUE_TYPE_INET) {
        // Parse "A.B.C.D" → 4-byte IPv4
        CassInet inet;
        unsigned a = 0, b = 0, c = 0, d = 0;
        std::sscanf(str.c_str(), "%u.%u.%u.%u", &a, &b, &c, &d);
        inet.address[0] = static_cast<uint8_t>(a);
        inet.address[1] = static_cast<uint8_t>(b);
        inet.address[2] = static_cast<uint8_t>(c);
        inet.address[3] = static_cast<uint8_t>(d);
        inet.address_length = 4;
        cass_statement_bind_inet(stmt, index, inet);
        break;
      } else if (exp == CASS_VALUE_TYPE_UUID || exp == CASS_VALUE_TYPE_TIMEUUID) {
        CassUuid uuid;
        cass_uuid_from_string(str.c_str(), &uuid);
        cass_statement_bind_uuid(stmt, index, uuid);
        break;
      } else if (exp == CASS_VALUE_TYPE_DECIMAL) {
        // Parse decimal string "123456.78" → scale + unscaled varint
        int32_t scale = 0;
        auto dot = str.find('.');
        std::string digits;
        if (dot != std::string::npos) {
          scale = static_cast<int32_t>(str.size() - dot - 1);
          digits = str.substr(0, dot) + str.substr(dot + 1);
        } else {
          digits = str;
        }
        ...
        if (nz == std::string::npos) digits = "0";
        else digits = digits.substr(nz);
        // Parse to int64 and encode as big-endian varint
        int64_t unscaled = 0;
        for (char c : digits) {
        ...
        uint8_t varint_bytes[8];
        for (int b = 0; b < 8; ++b) {
          varint_bytes[b] = static_cast<uint8_t>((unscaled >> (56 - b * 8)) & 0xFF);
        }
        // Find minimal varint representation
        size_t start = 0;
        while (start < 7) {
          uint8_t cur = varint_bytes[start];
          uint8_t next_high = varint_bytes[start + 1] & 0x80;
          if (cur == 0x00 && next_high == 0x00) { ++start; continue; }
          if (cur == 0xFF && next_high == 0x80) { ++start; continue; }
          break;
        }
        size_t varint_len = 8 - start;
        cass_statement_bind_decimal(stmt, index, varint_bytes + start, varint_len, scale);
        break;
      } else if (exp == CASS_VALUE_TYPE_DURATION) {
        // Parse duration "NmoNdNhNmNs" → months, days, nanoseconds
        int64_t months = 0, days = 0, hours = 0, mins = 0, secs = 0;
        ...
        while (*p) {
          ...
          if (std::strncmp(p, "mo", 2) == 0) { months = num; p += 2; }
          else if (*p == 'd') { days = num; ++p; }
          else if (*p == 'h') { hours = num; ++p; }
          else if (*p == 'm') { mins = num; ++p; }
          else if (*p == 's') { secs = num; ++p; }
          else { ++p; }
        }
        int64_t nanos = (hours * 3600 + mins * 60 + secs) * 1000000000LL;
        // Encode as 3 zigzag-encoded varints
        auto zigzag = [](int64_t v) -> uint64_t {
          return static_cast<uint64_t>((v << 1) ^ (v >> 63));
        };
        auto encode_vint = [](uint8_t *buf, uint64_t v) -> size_t {
          if (v < 128) { buf[0] = static_cast<uint8_t>(v); return 1; }
          // Count bytes needed
          int nbytes = 0;
          uint64_t tmp = v;
          while (tmp > 0) { ++nbytes; tmp >>= 8; }
          buf[0] = static_cast<uint8_t>(~((1 << (8 - nbytes)) - 1)); // header byte
          buf[0] |= static_cast<uint8_t>((v >> (8 * (nbytes - 1))) & ((1 << (8 - nbytes)) - 1));
          for (int i = 1; i < nbytes; ++i) {
            buf[i] = static_cast<uint8_t>((v >> (8 * (nbytes - 1 - i))) & 0xFF);
          }
          return static_cast<size_t>(nbytes);
        };
        uint8_t dur_buf[30];
        size_t pos = 0;
        pos += encode_vint(dur_buf + pos, zigzag(months));
        pos += encode_vint(dur_buf + pos, zigzag(days));
        pos += encode_vint(dur_buf + pos, zigzag(nanos));
        cass_statement_bind_bytes(stmt, index, dur_buf, pos);
        break;
      }
    }
...
// Overload to avoid string construction in hot paths
void encode_column_metadata(Buffer &buf, const char *keyspace, size_t ks_len,
                            const char *table, size_t tbl_len,
                            const char *column_name, size_t col_len,
                            TypeCode type) {
  buf.write_string(keyspace, ks_len);
  buf.write_string(table, tbl_len);
  buf.write_string(column_name, col_len);
  buf.write_short(static_cast<uint16_t>(type));
}
...
Buffer encode_rows_result(const CassResult *result, uint64_t latency_ns) {
  ...
  // Pre-allocate buffer: header (4) + flags (4) + col_count (4) +
  // column metadata (~50 bytes per column) + row_count (4) +
  // estimated row data (~100 bytes per cell) + latency (8)
  size_t estimated_size = 20 + (column_count * 50) + (row_count * column_count * 100) + 8;
  Buffer body(estimated_size);
  ...
  for (size_t i = 0; i < column_count; ++i) {
    ...
    encode_column_metadata(body, "", 0, "", 0, name, name_len, type_code);
  }
