...
class Buffer {
public:
  // Default capacity for small messages (avoids initial reallocations)
  static constexpr size_t DEFAULT_CAPACITY = 256;

  Buffer() { data_.reserve(DEFAULT_CAPACITY); }
  explicit Buffer(size_t capacity) { data_.reserve(capacity); }
  explicit Buffer(std::vector<uint8_t> data) : data_(std::move(data)) {}

  // Factory method for creating buffer with specific capacity hint
  static Buffer with_capacity(size_t capacity) {
    Buffer buf;
    buf.data_.reserve(capacity);
    return buf;
  }
  ...
  // Skip n bytes in read buffer
  void skip(size_t n) {
    if (read_pos_ + n > data_.size()) {
      throw std::runtime_error("Buffer underflow in skip");
    }
    read_pos_ += n;
  }
  ...
  void write_string(const char *s, size_t len) {
    write_short(static_cast<uint16_t>(len));
