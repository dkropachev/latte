...
// Strip port from contact points string (e.g. "127.0.0.1:9042,10.0.0.1:9042"
// -> hosts="127.0.0.1,10.0.0.1", port=9042).
// cass_cluster_set_contact_points() expects hostnames without ports;
// the port must be set separately via cass_cluster_set_port().
static std::pair<std::string, int> strip_ports_from_contact_points(
    const std::string &contact_points) {
  int port = -1;
  std::string hosts;
  std::istringstream stream(contact_points);
  std::string token;
  bool first = true;
  ...
  while (std::getline(stream, token, ',')) {
    ...
    // Check for host:port format (but not IPv6 bare addresses like ::1)
    size_t colon = token.rfind(':');
    if (colon != std::string::npos && colon > 0) {
      // Verify everything after the colon is digits (a port number)
...
Session::Session(uint64_t id, CassSession *session, CassCluster *cluster)
    : id_(id), session_(session), cluster_(cluster),
      prepared_cache_(std::make_shared<PreparedCacheMap>()) {}

Session::~Session() {
  // Free all prepared statements
  auto cache = std::atomic_load(&prepared_cache_);
  if (cache) {
    for (auto &[key, prepared] : *cache) {
...
// Lock-free read via atomic load (no mutex acquired on read path)
...
// Copy-on-write for cache updates (mutex only serializes writers)
void Session::cache_prepared(const std::string &key,
                             const CassPrepared *prepared) {
  std::lock_guard<std::mutex> lock(cache_write_mutex_);

  // Copy current map
  auto old_cache = std::atomic_load(&prepared_cache_);
  auto new_cache = std::make_shared<PreparedCacheMap>(*old_cache);

  // Modify copy
  (*new_cache)[key] = prepared;

  // Atomically swap in new map
  std::atomic_store(&prepared_cache_, new_cache);
}
...
CassCluster *SessionRegistry::create_cluster(
  ...
  // Contact points - strip ports since cass_cluster_set_contact_points()
  // expects hostnames only; port is set separately via cass_cluster_set_port()
  std::string contact_points;
  auto it = params.find("contact_points");
  if (it != params.end() && !it->second.empty()) {
    contact_points = it->second;
  }
  auto [hosts, port] = strip_ports_from_contact_points(contact_points);
