...
// Compile: g++ -std=c++17 -I../src tests/test_protocol.cpp ../src/protocol.cpp -o test_protocol
...
void test_frame_header_encode() {
    std::cout << "Testing frame header encoding via encode_frame..." << std::endl;

    // encode_frame builds a full frame (header + body); verify the header portion
    Buffer body;
    // Create a body of exactly 100 bytes
    for (int i = 0; i < 100; i++) body.write_byte(0);

    Buffer encoded = encode_frame(42, Opcode::RESULT, body);

    assert(encoded.size() == FRAME_HEADER_SIZE + 100);
...
void test_build_schema_change_result() {
    ...
    Buffer body = build_schema_change_result("CREATED", "TABLE", "ks", "my_table");
