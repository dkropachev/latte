...
|-----------|-----------|-------|
| `contact_points` | Yes | Comma-separated host:port list |
...
- **`vector<float, N>` not supported**: The cpp-rs-driver does not implement `cass_statement_bind_custom()`, which is required to bind data to vector columns. Vector test suites are skipped. See [scylladb/cpp-rust-driver#415](https://github.com/scylladb/cpp-rust-driver/issues/415).
