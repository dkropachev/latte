...
| Option | Description |
|--------|-------------|
| `--cql-adapter-image <IMAGE>` | Docker image to use for the driver adapter. When specified, Latte starts a container and delegates query execution to it via Unix domain socket. Example: `--cql-adapter-image scylladb/latte-driver-adapters:scylla-rust-driver-1.4.0` |
| `--driver-socket <PATH>` | Path to the Unix domain socket for driver communication. Default: `/tmp/latte-driver.sock`. Without `--cql-adapter-image`: connects to an existing driver at this socket. With `--cql-adapter-image`: specifies where the driver should create its socket. |

### Usage Examples

```bash
# Use Docker image (Latte manages container lifecycle)
latte run --cql-adapter-image scylladb/latte-driver-adapters:scylla-rust-driver-latest workload.rn
...
latte run --cql-adapter-image scylladb/latte-driver-adapters:scylla-rust-driver-1.4.0 \
...
| gocql | ✅ | ✅ | ✅ | ✅ | Full support |
| python-driver | ✅ | ✅ | ✅ | ❌ | Vectors: adapter bug ([details](#python-driver)) |
| csharp-driver | ✅ | ✅ | ✅ | ✅ | Full support |
| cpp-rs-driver | ✅ | ✅ | ✅ | ❌ | Vectors: driver limitation ([details](#cpp-rs-drivervectors)) |
| cpp-driver | ✅ | ✅ | ❌ | ✅ | Collections: driver limitation ([details](#cpp-drivercollections)) |

Legend:
- ✅ Supported and tested
- ❌ Not supported (skipped in automated tests)

## Known Issues

See [KNOWN-ISSUES.md](KNOWN-ISSUES.md) for detailed information about known driver and adapter limitations affecting workload compatibility.
...
```bash
# Profile from cql-adapters/ directory
make profile
make profile-gocql
make profile-java
...
|---------|----------|----------|
| scylla-rust-driver | `cql-adapters/scylla-rust-driver/` | Rust |
| java-driver-4 | `cql-adapters/java-driver-4/` | Java |
| java-driver-3 | `cql-adapters/java-driver-3/` | Java |
| gocql | `cql-adapters/gocql/` | Go |
| python-driver | `cql-adapters/python-driver/` | Python |
| csharp-driver | `cql-adapters/csharp-driver/` | C# |
| cpp-driver | `cql-adapters/cpp-driver/` | C++ |
| cpp-rs-driver | `cql-adapters/cpp-rs-driver/` | Rust bindings |

### Quick Start

```bash
cd cql-adapters/scylla-rust-driver
